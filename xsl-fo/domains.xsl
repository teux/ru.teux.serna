...
<!--  Redefines domains2fo.xsl for conditional attributes visualisation -->
...
<xsl:template match="*[contains(@class,' pr-d/fragment ')]/*[contains(@class,' topic/title ')]" dtm:id="pr-d.fragment.title">
	<fo:block font-weight="bold"><xsl:call-template name="ditavalAttrs"/><xsl:apply-templates/></fo:block>
</xsl:template>
...
<fo:inline border-left-width="0pt" border-right-width="0pt" font-family="Courier">
  ...
  &lt;<fo:block><xsl:call-template name="ditavalAttrs"/><xsl:apply-templates/></fo:block>&gt;<!--/a-->
</fo:inline>
</xsl:template>
